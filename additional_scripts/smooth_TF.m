...
function [data_smoothed]=smooth_TF(cfg,data)

if strncmp(data.dimord,'rpt_chan_freq_time',numel(data.dimord));
else
    error('dimord does not match')
end
...
powspctrm(trial,chan,:,:)=imfilter(squeeze(powspctrm(trial,chan,:,:)),h,'conv'); 
end

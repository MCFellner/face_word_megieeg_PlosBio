...
function [data_smoothed]=smooth_TF_GA(cfg,data)
...
for chan=1:size(powspctrm,1)
% use imfilter, conv2 smears data to different tf bins, and is slower!
%w = conv2(squeeze(freq.powspctrm(1,1,:,:)),h); 
powspctrm(chan,:,:)=imfilter(squeeze(powspctrm(chan,:,:)),h,'conv'); 
%end

display(strcat ('smoothed trial ',num2str(chan)));
end
